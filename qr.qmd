...
where  
    ...
    - $\mathbf{R} \in \mathbb{R}^{n \times p}$  is upper triangular with positive diagonal entries. (The positivity constraint can be relaxed; see Householder QR below.) 
...
<img src="https://upload.wikimedia.org/wikipedia/commons/f/f0/Alston_Householder.jpg" width="200" align="center"/>
...
* Typically we require solution $\boldsymbol{\beta}$ has a smallest norm; the solution depends on the choice of the norm.
...
$$
    - $\|\boldsymbol{\beta}\|_0 \triangleq \sum_{i=1}^p \mathbf{1}_{\{\beta_i\neq 0\}}$, the "sparsest" solution.
